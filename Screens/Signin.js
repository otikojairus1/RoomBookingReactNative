...
const onSubmitHandler = (event) => {
  ...
  axios({
    method: 'post',
    url: 'https://hotelbooking1api.herokuapp.com/api/login',
    data: {
      "email":email,
      "password":password
  },
    config: { headers: {'Content-Type': 'application/json' }}
     ...
     .then(function (response){
      ...
      if(typeof response.data.error !== "undefined"){
        ...
        //console.log(response.data.userDetails.email);
     if (response.data.userDetails.email == "admin@room.com"){


navigation.navigate('Create Room', {identity: response.data.userDetails.email});
     }else{
      navigation.navigate('Rooms Available', {identity: response.data.userDetails.email});
     }
